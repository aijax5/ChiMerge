...
# 5. go 3 until chi2 values from all pairs exceed threshold of pre-defined significance level (until all alternative hypothesises become passed)

class ChiMerge(Discretizer):
    def __init__(self, con_features:List[str], significance_level=0.1, max_cutpoints=3, n_jobs=1):
        # significance_level = 0.1, 0.05, 0.01... etc
        assert 1 > significance_level > 0, 'significance level should be (0,1)'
        assert max_cutpoints > 1, 'the number of max cutpoints should be bigger than 1'
